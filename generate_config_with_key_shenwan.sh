...
if [ -z "$NODE_ROOT" ]; then
    ...
    echo "说明: 申万宏源 DolphinDB 适配版，会写入 Master 侧沪深 DB/table 校验配置。"
    exit 1
fi
...
for node_file in $(ls "$NODE_ROOT"); do
  ...
  if [ -f "$node_config_path" ] && [ -r "$node_config_path" ]; then
...
echo "Shenwan DolphinDB routing: market DB/table"
echo ""
...
for node_file in $(ls "$NODE_ROOT"); do
  if [ -d "$NODE_ROOT/$node_file" ]; then
    ...
    if [ -f "$node_config_path" ] && [ -r "$node_config_path" ]; then
        ...
        NODE_ABM_STOCK_DATA_DIR=$(awk -F'"' '/"ABMStockDataDir"/ {print $4; exit}' "$node_config_path")
        NODE_ABM_STOCK_PARAM_DIR=$(awk -F'"' '/"ABMStockParamDir"/ {print $4; exit}' "$node_config_path")
        if [ -n "$NODE_ABM_STOCK_DATA_DIR" ] && [ "$NODE_ABM_STOCK_DATA_DIR" != "$ABM_STOCK_DATA_DIR" ]; then
            echo "  - Warning: NODE($node_file) ABMStockDataDir differs: $NODE_ABM_STOCK_DATA_DIR"
        fi
        if [ -n "$NODE_ABM_STOCK_PARAM_DIR" ] && [ "$NODE_ABM_STOCK_PARAM_DIR" != "$ABM_STOCK_PARAM_DIR" ]; then
...
EOL
        ...
        if [ ! -f "$spec_key_file" ] || [ ! -r "$spec_key_file" ]; then
            echo "NODE($node_file) spec public key not found!"
            exit 1
        fi
        if [ ! -f "$bls_key_file" ] || [ ! -r "$bls_key_file" ]; then
